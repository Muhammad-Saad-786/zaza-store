...
function PixelatedImageTrail({
  ...
  useEffect(() => {
    ...
    const getSliceDelay = (index, stagger) =>
    ...
    const distanceFromLastSpawn = () => {
      const dx =
        interpolatedPointerPosRef.current.x - lastSpawnPosRef.current.x;
      const dy =
        interpolatedPointerPosRef.current.y - lastSpawnPosRef.current.y;
  ...
  return (
    <div
      ...
      aria-hidden="true"
      className={cn(
        "absolute inset-0 overflow-hidden pointer-events-auto touch-none",
        className,
      )}
    />
  );
